...
func scanInvite(sc interface{ Scan(...any) error }) (store.TenantInvite, error) {
	...
	i.ExpiresAt, i.CreatedAt = i.ExpiresAt.UTC(), i.CreatedAt.UTC()
	return i, nil
}
...
func (d *DB) CreateInvite(ctx context.Context, tenantID string, inv *store.TenantInvite) error {
	...
	_, err := d.db.ExecContext(ctx, `INSERT INTO tenant_invites (id, tenant_id, email_lower, role, token_hash, expires_at, accepted_at, created_at) VALUES (?, ?, ?, ?, ?, ?, NULL, ?)`,
...
func (d *DB) GetPendingInviteByEmail(ctx context.Context, email string) (*store.TenantInvite, error) {
	i, err := scanInvite(d.db.QueryRowContext(ctx, "SELECT "+inviteCols+" FROM tenant_invites WHERE email_lower=? AND accepted_at IS NULL AND expires_at > ? ORDER BY created_at DESC LIMIT 1",
		strings.ToLower(strings.TrimSpace(email)), time.Now().UTC()))
	if err != nil {
		return nil, err
	}
	return &i, nil
...
func (d *DB) AcceptInvite(ctx context.Context, id string) error {
	_, err := d.db.ExecContext(ctx, "UPDATE tenant_invites SET accepted_at=? WHERE id=? AND accepted_at IS NULL", time.Now().UTC(), id)
	return err
}
