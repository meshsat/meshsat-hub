...
func scanTenant(sc interface{ Scan(...any) error }) (store.Tenant, error) {
	...
	if err := sc.Scan(&t.ID, &t.Slug, &t.Name, &t.OwnerUserID, &t.Plan, &t.Status, &created, &updated, &deleted, &expires, &t.KofiClaimCode, &t.KofiPayerEmail, &t.KofiLastMessageID, &warned, &t.BillingCountry, &t.BillingCountryEvidence, &t.StripeCustomerID, &t.StripeSubscriptionID, &t.BridgeOfflineTimeout); err != nil {
		return t, err
	}
...
func (d *DB) UpdateTenant(ctx context.Context, t *store.Tenant) error {
	...
	_, err := d.db.ExecContext(ctx, `UPDATE tenants SET slug=?, name=?, owner_user_id=?, plan=?, status=?, updated_at=?, plan_expires_at=?, kofi_claim_code=?, kofi_payer_email=?, kofi_last_message_id=?, lapse_warned_at=?, billing_country=?, billing_country_evidence=?, stripe_customer_id=?, stripe_subscription_id=?, bridge_offline_timeout=? WHERE id=?`,
		t.Slug, t.Name, t.OwnerUserID, t.Plan, t.Status, fmtTime(t.UpdatedAt), fmtTimePtr(t.PlanExpiresAt), t.KofiClaimCode, t.KofiPayerEmail, t.KofiLastMessageID, fmtTimePtr(t.LapseWarnedAt), t.BillingCountry, t.BillingCountryEvidence, t.StripeCustomerID, t.StripeSubscriptionID, t.BridgeOfflineTimeout, t.ID)
	return err
}
